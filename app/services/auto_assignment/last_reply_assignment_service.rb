...
class AutoAssignment::LastReplyAssignmentService
  ...
  def find_assignee
    ...
    if agent.present?
      Rails.logger.info(
        "[AUTO_ASSIGNMENT] service=last_reply conversation_id=#{conversation.id} strategy=matched candidate_user_id=#{agent.id}"
      )
      return agent
    end

    Rails.logger.info(
      "[AUTO_ASSIGNMENT] service=last_reply conversation_id=#{conversation.id} strategy=fallback_round_robin allowed_online_agent_ids=#{allowed_online_agent_ids.inspect}"
    )
  ...
  def last_reply_agent
    current_candidate = current_conversation_last_reply_agent_id
    historical_candidate = historical_last_reply_agent_id
    candidate_agent_id = current_candidate || historical_candidate

    Rails.logger.info(
      "[AUTO_ASSIGNMENT] service=last_reply conversation_id=#{conversation.id} current_candidate=#{current_candidate.inspect} historical_candidate=#{historical_candidate.inspect} historical_conversation_ids=#{historical_conversation_ids.to_a.inspect} online_agent_ids=#{online_agent_ids.inspect}"
    )
